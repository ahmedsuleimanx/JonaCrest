...
const Companies = () => {
    ...
    const fetchPartners = async () => {
      try {
        ...
        if (res.data.success && res.data.partners?.length > 0) {
          setApiPartners(res.data.partners);
        }
      } catch (err) {
        // Silently fail — fall back to static logos
      }
    };
    fetchPartners();
  }, []);
  ...
  // Use API partners if available, otherwise fall back to static logos
  const companyLogos = apiPartners.length > 0
    ? apiPartners.map(p => ({ src: p.logo, alt: p.name, name: p.name, website: p.website }))
    : staticLogos;
