...
class FirecrawlService {
    constructor() {
        ...
        if (config.firecrawlApiKey && config.firecrawlApiKey.length > 10) {
            try {
            ...
            console.log("⚠️ Firecrawl not configured - AI property search disabled. Set FIRECRAWL_API_KEY");
        }
    }

    async findProperties(city, maxPrice, propertyCategory = "Residential", propertyType = "Flat", limit = 6) {
        if (!this.isConfigured) {
            return { properties: [], error: "Firecrawl not configured" };
        }
        try {
