  ...
  const fetchProperties = async () => {
    try {
      ...
      if (response.data.success) {
        // Backend returns 'products' not 'property'
        const propertiesData = response.data.products || response.data.property || [];
  ...
  const filteredProperties = properties
    .filter(property => {
      const matchesSearch = !searchTerm || 
        [property.title, property.location, property.type || property.propertyType]
          .filter(Boolean)
          .some(field => field?.toLowerCase().includes(searchTerm.toLowerCase()));
      
      const propType = property.type || property.propertyType || '';
      const matchesType = filterType === "all" || propType.toLowerCase() === filterType.toLowerCase();
